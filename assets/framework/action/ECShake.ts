...
@ccclass
export  class ECShake extends cc.ActionInterval
{
     ...
     * @returns {Shake}
     */
    public static create(duration:number,strength_x:number,strength_y:number):ECShake
    {
        let act:ECShake = new ECShake();
        act.initWithDuration( duration,strength_x,strength_y );
        return act;
    }
 
    initWithDuration(duration:number,strength_x:number,strength_y:number):boolean
    {
    ...
    fgRangeRand(min:number,max:number):number
    {
